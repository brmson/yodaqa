...
public class LATByFocus extends JCasAnnotator_ImplBase {
	...
	protected void addFocusLAT(JCas jcas, Focus focus) {
		...
		// TODO: synsety jsou anglicky!!!
		if (text.equals("kdo") || text.equals("čí")) {
			...
			addFocusLAT(jcas, focus, "osoba", null, 7846, 0.0, new QuestionWordLAT(jcas));

		} else if (text.equals("kdy")) {
			...
			addFocusLAT(jcas, focus, "čas", null, 15147173, 0.0, new QuestionWordLAT(jcas));
			...
			addFocusLAT(jcas, focus, "datum", null, 15184543, 0.0, new QuestionWordLAT(jcas));

		} else if (text.equals("kde")) {
			/* (992){00027365} <noun.Tops>[03] S: (n) location#1 (location%1:03:00::) (a point or extent in space) */
			addFocusLAT(jcas, focus, "místo", null, 27365, 0.0, new QuestionWordLAT(jcas));

		} else if (text.equals("kolik") || text.equals("moc")) {
			...
			addFocusLAT(jcas, focus, "množství", null, 33914, 0.0, new QuestionWordLAT(jcas));
			...
			// TODO: just a sequence of JJs + NNs
	...
	public static boolean isAmbiguousQLemma(String text) {
		return text.matches("^co|proč|jak|jaký|který|jméno$");
	}
}
